...
const Categorys = () => {
  ...
  return (
    <div onMouseLeave={() => setActive(null)} className='pt-10 pe-4 h-fit border-e border-border hidden lg:block'>
      <div className='flex flex-col items-start p-0 gap-4 w-[217px] '>
        {categorys.map((category, i) => (
          <Category key={i} category={category} onHover={() => onhoverCat(category)} />
        ))}
      </div>
      <DropdownPanel
        items={activeItems}
