...
const ImageSliderContext = createContext<imageSliderContextType | undefined>(
  undefined
);
...
export const useImageSliderContext = () => {
  ...
  if (context === undefined) {
    throw new Error(
      "useImageSliderContext must be used within ImageSliderProvider"
    );
  }
  return context;
};
...
export const ImageSliderProvider = ({
  ...
  const setCurrent =
    setCurrentProp === undefined ? setCurrentState : setCurrentProp;
...
export const ImageSliderWrapper = ({
  ...
  <ImageSliderProvider {...{ current, setCurrent }}>
    <div
      className={cn(
        "!relative w-full h-[344px] overflow-hidden bg-black",
        className
      )}
    >
      {children}
      ...
      {sliderArr.map((_, index) => {
        ...
        return (
          <motion.button
            ...
            aria-label={`Slider progress circle ${index + 1}`}
