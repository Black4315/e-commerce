...
const StickyBtns = ({ hide }: { hide: boolean }) => {
    ...
    return (
      <>
        <ul
          className={`fixed bottom-3 start-3 lg:bottom-8 lg:start-8 z-60 flex flex-col gap-4 bg-white/80 backdrop-blur-sm border-border border rounded-xl py-2 px-1 md:py-3 md:px-2
        ...
        >
          <ActionsNav.wishList
            tooltip={false}
            iconClassName={`${animateW && "animate__heart"}`}
          />
          <ActionsNav.cart
            tooltip={false}
            iconClassName={`${animateC && "animate_cartBump"}`}
          />
        </ul>
