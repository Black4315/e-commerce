...
const SliderItem = ({ name, image, logo, offerBody, link }: SliderItemtype) => {
    ...
    return (
      <div
        key={name}
        className="flex p-6 pb-12 sm:pb-14 sm:p-6 md:p-12 box-border h-full justify-between max-md:flex-col relative"
      >
        <div className="max-md:max-h-50 max-sm:w-63 w-75 md:order-2 max-md:self-center md:absolute md:end-12">
          <Image
            src={image}
            width={350}
            height={50}
        ...
        <div className="flex flex-col max-w-75 gap-1 sm:gap-4 md:gap-6 z-1">
          <LogoTitle
            {...{
              logo,
              name,
          ...
          <Link
            href={link}
            className="flex items-center gap-2 text-text-1 hover:text-hover-button-1 group transition-colors"
          >
