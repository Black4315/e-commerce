        ...
        <time dateTime={endDate.toString()} className="flex items-end relative">
            {Object.entries(timeLeft).map(([key, value], index) => (
                <React.Fragment key={key}>
                    <div className={`flex flex-col gap-3 relative ${(timeLeft.hours <= 1 && timeLeft.days <= 0) && 'text-danger-700 text-shadow-danger-500 text-shadow-[0_0_7px_text-shadow-danger-500]'} ${(timeLeft.hours <= 1 && timeLeft.days <= 0 && !end) && 'animate-pulse'}`}>
